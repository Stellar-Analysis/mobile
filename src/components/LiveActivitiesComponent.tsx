...
export const LiveActivitiesComponent: React.FC = () => {
  const { isSupported, loading, error, activities, startActivity, endActivity, clearActivities } =
    useLiveActivities();
  ...
  return (
    <ScrollView
      ...
      accessibilityLabel="Live Activities screen">
      ...
      <TextInput
        ...
        accessibilityLabel="Activity subtitle input"
      />
      <Button
        title="Start Activity"
        onPress={() => {
          void startActivity(title, subtitle);
          setTitle('');
          setSubtitle('');
        }}
        disabled={!isSupported || loading || !title}
      ...
      <View
        ...
        accessibilityLabel="Live Activities log">
        <View style={styles.row}>
          <Text style={styles.sectionTitle}>Activities</Text>
          {activities.length > 0 ? (
            <Button
              title="Clear"
              onPress={() => void clearActivities()}
              accessibilityLabel="Clear activities"
            />
          ) : null}
        </View>
        {activities.length === 0 ? (
            ...
            <View
              ...
              <View>
                ...
                <Text style={styles.meta}>
                  {a.subtitle} · {a.status} · {new Date(a.startedAt).toLocaleTimeString()}
                </Text>
              </View>
              {a.status === 'active' ? (
                <Button
                  title="End"
                  onPress={() => void endActivity(a.id)}
                  accessibilityLabel={`End ${a.title}`}
                />
              ) : (
