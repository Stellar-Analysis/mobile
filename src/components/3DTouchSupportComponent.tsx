...
export const TouchSupportComponent: React.FC = () => {
  const { supported, loading, error, isOffline, actions, lastForce, handlePress } =
    use3DTouchSupport();

  React.useEffect(() => {
    if (error) {
      Alert.alert('3D Touch Error', error, [{ text: 'OK' }], { cancelable: true });
    }
  }, [error]);
  ...
  return (
    <ScrollView
      ...
      accessibilityLabel="3D Touch support screen">
      ...
      <View style={styles.body}>
        <Text style={styles.status} accessibilityRole="text">
          {supported
            ? '3D Touch is supported on this device.'
            : '3D Touch is not available on this platform.'}
        </Text>
        ...
        <TouchableOpacity
          ...
          {forcePercent !== null && <Text style={styles.forceText}>Force: {forcePercent}%</Text>}
        </TouchableOpacity>
        ...
        <View
          ...
          accessibilityLabel="Available 3D Touch actions">
