    ...
    <ScrollView contentContainerStyle={styles.container} accessibilityLabel="App Clips screen">
      ...
      {SAMPLE_CLIPS.map(clip => (
        <View key={clip.url} style={styles.itemRow}>
          ...
          <Button
            title="Launch"
            onPress={() => void launchClip(clip.url, clip.title)}
            disabled={!isSupported || loading}
            accessibilityLabel={`Launch ${clip.title}`}
          />
        </View>
      ))}
      ...
      <TextInput
        ...
        accessibilityLabel="Clip URL input"
      />
      <TextInput
        style={styles.input}
        placeholder="Title"
        ...
        accessibilityLabel="Clip title input"
      />
      <Button
        title="Launch Custom Clip"
        onPress={() => {
          void launchClip(customUrl, customTitle);
          setCustomUrl('');
          setCustomTitle('');
        }}
        disabled={!isSupported || loading || !customUrl}
        accessibilityLabel="Launch custom clip"
      />

      <View
        style={styles.logCard}
        accessibilityRole="summary"
        accessibilityLabel="App Clips launch log">
        <View style={styles.row}>
          <Text style={styles.sectionTitle}>Launch Log</Text>
          {clips.length > 0 ? (
            <Button
              title="Clear"
              onPress={() => void clearClips()}
              accessibilityLabel="Clear clips log"
            />
          ) : null}
        </View>
        {clips.length === 0 ? (
          ...
          clips.map(c => (
            <View
              ...
              <Text style={[styles.itemName, c.status === 'failed' && styles.failed]}>
                {c.title}
              </Text>
              <Text style={styles.meta}>
                {c.status} · {new Date(c.launchedAt).toLocaleTimeString()}
              </Text>
            </View>
          ))
        )}
